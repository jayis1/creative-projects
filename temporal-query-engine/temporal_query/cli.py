...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps([e.__dict__ if hasattr(e,'__dict__') else {'id':e.id,'start':e.start,'end':e.end,'label':e.label} for e in idx.overlaps(args.start,args.end)], sort_keys=True))
        return 0
