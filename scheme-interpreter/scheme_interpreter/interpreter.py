...
class Interpreter:
    ...
    def __init__(self):
        ...
        self._traced = {}  # name -> (original_proc, wrapper)
        # Auto-load standard library
        self._load_stdlib()

    def _load_stdlib(self):
        """Load the bundled stdlib.scm into the global environment."""
        import os
        ...
        try:
            ...
            import sys
            sys.stderr.write(f"Warning: failed to load stdlib: {e}\n")
    ...
    def _make_load(self):
        ...
        def load_proc(path):
            if not isinstance(path, str):
                raise SchemeError("load: expected a string path")
            try:
                with open(path) as f:
                    source = f.read()
            except OSError as e:
                raise SchemeError(f"load: {e}")
            return interp.run(source, interp.global_env)
        return Procedure("load", load_proc)

    def _make_trace(self):
        interp = self
        def trace_proc(name_or_proc):
            # If given a symbol, look up the procedure
            ...
            # Save original and create wrapper
            if name in interp._traced:
                return proc  # already traced
            original = proc
            def traced_wrapper(*args):
                import sys
                arg_strs = " ".join(scheme_repr(a) for a in args)
                sys.stderr.write(f"-> ({name} {arg_strs})\n")
                result = interp._apply(original, list(args))
                sys.stderr.write(f"<- {name} = {scheme_repr(result)}\n")
                return result
            wrapper = Procedure(f"trace<{name}>", traced_wrapper)
            interp._traced[name] = (original, wrapper)
            if isinstance(name_or_proc, Symbol):
    ...
    def _make_untrace(self):
        ...
        def untrace_proc(name_or_proc):
            if isinstance(name_or_proc, Symbol):
                ...
                name = getattr(name_or_proc, 'name', 'unknown')
            else:
                raise SchemeError("untrace: expected a symbol or procedure")
            if name in interp._traced:
                original, _ = interp._traced.pop(name)
                interp.global_env.define(name, original)
                return original
    ...
    def _make_time(self):
        ...
        def time_proc(thunk):
            ...
            import sys
            sys.stderr.write(f"Elapsed: {elapsed:.6f}s\n")
            return result
        return Procedure("time", time_proc)

    def _make_assert(self):
        interp = self
        def assert_proc(expr, *message):
            if not is_true(expr):
                msg = scheme_display(message[0]) if message else "assertion failed"
                raise SchemeError(f"assert: {msg}")
            return Unspecified
...
# ---------------------------------------------------------------------------
# additional special forms (must be defined before the dispatch table)

def _sf_rec(interp, expr, env):
    """(rec name expr) — define a recursive named expression.

    Like (letrec ((name expr)) name) but as a single expression.
    """
    name = expr.cdr.car
    if not isinstance(name, Symbol):
        raise SchemeError("rec: name must be a symbol")
    body_expr = expr.cdr.cdr.car
    new_env = Environment(parent=env)
    new_env.define(name.name, Unspecified)
    val = interp.seval(body_expr, new_env)
    new_env.define(name.name, val)
    return val


def _sf_let_values(interp, expr, env):
    """(let-values ((var-list expr) ...) body ...) — destructuring let.

    Each binding's expr is evaluated and should produce multiple values
    via (values ...) which are destructured into the corresponding vars.
    """
    bindings = pairs_to_list(expr.cdr.car)
    new_env = Environment(parent=env)
    for b in bindings:
        var_list = pairs_to_list(b.car)
        val_expr = b.cdr.car
        result = interp.seval(val_expr, env)
        if isinstance(result, MultipleValues):
            vals = result.values
        else:
            vals = [result]
...
def _sf_define_values(interp, expr, env):
    """(define-values (var ...) expr) — define multiple values."""
    var_list = pairs_to_list(expr.cdr.car)
    val_expr = expr.cdr.cdr.car
    result = interp.seval(val_expr, env)
    if isinstance(result, MultipleValues):
        vals = result.values
    else:
