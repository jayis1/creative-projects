...
on a gradient background, then demonstrates:
  1. Energy map visualization (all 5 energy functions)
...
def main() -> int:
    ...
    print("=== Seam Carving Demo (Enhanced) ===\n")
    ...
    # 2. Energy maps for all 5 energy functions
    ...
    for etype in EnergyType:
        ...
        fname = f"energy_{etype.value}.ppm"
        write_ppm(os.path.join(outdir, fname), emap_rgb)
    ...
    write_ppm(os.path.join(outdir, "multi_seam_visualization.ppm"), vis_multi)
    print(f"   Saved: output/multi_seam_visualization.ppm")
    ...
    write_ppm(os.path.join(outdir, "protected_carve.ppm"), result)
    print(f"   Saved: output/protected_carve.ppm ({result.shape[1]}x{result.shape[0]})")
    ...
    # 11. Prewitt and Laplacian energy comparison
    print("\n11. Prewitt & Laplacian energy carving (20 seams)...")
    for etype in [EnergyType.PREWITT, EnergyType.LAPLACIAN]:
        c = SeamCarver(img, energy_type=etype)
        result = c.carve_vertical(20)
        fname = f"carved_{etype.value}.ppm"
        write_ppm(os.path.join(outdir, fname), result)
