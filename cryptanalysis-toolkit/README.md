...
A comprehensive Python framework for implementing, analyzing, and breaking classical ciphers using statistical methods including frequency analysis, Kasiski examination, index of coincidence, and hill climbing.

## Features

### 10 Classical Cipher Implementations
...
- **Index of Coincidence** — IC calculation for cipher type identification; key length estimation for polyalphabetic ciphers; language identification
...
- **Cipher Type Identifier** — Uses IC and frequency statistics to classify unknown ciphertexts

## How It Works

### Cipher Implementations
Each cipher provides `encrypt()` and `decrypt()` methods that handle mixed case and pass through non-alphabetic characters unchanged. Ciphers validate their keys on construction and raise `ValueError` for invalid parameters.
...
The IC measures the probability that two randomly chosen letters are the same. For monoalphabetic ciphers, IC ≈ 0.0667 (same as English). For polyalphabetic ciphers, IC drops toward 0.0385 (random). This distinguishes cipher types and estimates Vigenère key lengths.
...
# Kasiski examination
k = KasiskiExaminer()
print(k.kasiski_report(ciphertext))
...
cryptanalysis break vigenere "ciphertext_here" --max-key-length 10
cryptanalysis break affine "ciphertext_here"
cryptanalysis break substitution "ciphertext_here" --iterations 5000
cryptanalysis break auto "ciphertext_here"

# Analyze
cryptanalysis analyze "ciphertext_here" --top-n 15
```
