...
def rs_decode_detailed(received: List[int], nsym: int,
    ...
    Like :func:`rs_decode` but returns a :class:`DecodeResult` with error/erasure
    ...
    if not erasures:
        ...
        logger.debug("Decoded %d errors (nsym=%d)", num_errs, nsym)
        ...
        logger.debug("Decoded %d erasures + %d errors (nsym=%d)",
                     len(err_positions), len(pure_errors), nsym)
