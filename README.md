...
| [waveform-synth](./waveform-synth) | Digital audio synthesizer v3.0: 7 waveform types, pulse oscillator, ADSR envelopes, FM synthesis with 5 presets, 10 effects (Schroeder reverb, compressor, delay, flanger, distortion, LP/HP/BP filters, tremolo, gain, echo), stereo processing, YIN pitch detection, spectral analysis, DSP primitives (FFT, Butterworth filters, windowing, convolution, onset detection), multi-format I/O (WAV/AIFF/raw PCM/MIDI), configuration system (JSON/TOML, 5 presets), composition engine, note/scale/chord library, ASCII visualization, argparse CLI (8 subcommands), pyproject.toml, CI, 160 tests |
